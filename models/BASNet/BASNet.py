...
from models.BASNet.resnet_model import *
...
class BASNet(nn.Module):
    def __init__(self,n_channels, kd=False):
        ...
        if self.kd:
            self.rfb1 = nn.Conv2d(64, 32, 1, padding=0)
            self.rfb2 = nn.Conv2d(64, 32, 1, padding=0)
            ...
            self.outconv3 = nn.Conv2d(32, 1, 3, padding=1)
            self.outconv2 = nn.Conv2d(32, 1, 3, padding=1)
            self.outconv1 = nn.Conv2d(32, 1, 3, padding=1)
        else:
    ...
    def forward(self, x):
        ...
        if self.kd:
            x1 = self.rfb1(hd1)
            att_1 = self.att_1(x1)
            d1 = x1 * F.sigmoid(att_1)

            x2 = self.rfb2(hd2)
            att_2 = self.att_2(x2)
            d2 = x2 * F.sigmoid(att_2)

            x3 = self.rfb3(hd3)
            att_3 = self.att_3(x3)
            d3 = x3 * F.sigmoid(att_3)
        ...
        if self.kd:
            att_1 = _upsample_like(att_1, d1)
            att_2 = _upsample_like(att_2, d1)
            att_3 = _upsample_like(att_3, d1)

            return dout, d1, d2, d3, d4, d5, d6, att_1, att_2, att_3

        return dout, d1, d2, d3, d4, d5, d6
